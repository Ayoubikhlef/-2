...
function detectInfoType(message, lang) {
...
async function processMessage(message, language) {
    ...
    const keywords = (0, nlp_1.extractKeywords)(message, lang);
    ...
    const scored = products.map(p => {
        const searchText = `${p[nameField]} ${p[descField]} ${p.brand || ''}`.toLowerCase();
        const score = keywords.filter(kw => searchText.includes(kw)).length;
        return { ...p, score };
    });
    scored.sort((a, b) => b.score - a.score);
    const matchedProducts = scored.slice(0, 5).map(p => ({
        id: p.id,
        nameAr: p.nameAr,
        nameFr: p.nameFr,
        nameEn: p.nameEn,
    ...
    const nameFieldShort = getFieldName(lang, 'name');
